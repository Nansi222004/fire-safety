...
const DeliveryPrivacyPolicy = () => {
    ...
    const fetchPolicyAndSettings = async () => {
      ...
      try {
        const [policyRes, settingsRes] = await Promise.allSettled([
          api.get('/policies/delivery-privacy'),
        ...
        console.error('Failed to load delivery privacy policy:', err);
      } finally {
  ...
  const fallbackContent = `
    <h2>1. Delivery Partner Data Collection</h2>
    <p>SafeFire collects partner identification, GPS geolocation during active duty, driving credentials, and order transit logs solely to assign delivery tasks, optimize routes, and ensure the safe transport of certified fire safety equipment.</p>
    <h2>2. Transit & Safety Records</h2>
    <p>All emergency delivery logs, safety inspection dispatches, and equipment handling verifications are stored securely to verify compliance with certified fire protection standards.</p>
    <h2>3. Location Tracking & Operational Privacy</h2>
    <p>Real-time location data is collected only when a delivery partner is toggled "Online" or executing an active order delivery/return pickup. SafeFire respects your privacy during offline hours.</p>
    <h2>4. Support & Partner Grievances</h2>
    <p>For any queries regarding your delivery partner account, transit records, or data protection, our dedicated partner support desk is available to assist you.</p>
  `;
      ...
      <div className="min-h-screen bg-[#F8FAFC] text-[#1F2937] flex flex-col justify-between selection:bg-red-500 selection:text-white">
        ...
        <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-[#E5E7EB] px-4 lg:px-8 py-3.5 shadow-xs">
          <div className="max-w-5xl mx-auto flex items-center justify-between">
            ...
            <button
              onClick={() => navigate(isAuthenticated ? '/delivery/dashboard' : '/delivery/login')}
        ...
        <main className="flex-1 max-w-4xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-50 text-red-600 border border-red-200 text-xs font-bold">
              ...
              <span>Partner Compliance & Privacy Policy</span>
            </div>
            {policy?.lastUpdated && (
              ...
              <div className="flex items-center gap-3 text-red-600 mb-2">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-extrabold text-[#0F172A] tracking-tight">
                    {policy?.title || 'Delivery Partner Privacy Policy'}
                  </h1>
                  <p className="text-xs sm:text-sm text-[#64748B] mt-0.5">
                    Operational privacy standards, location data security, and platform compliance
                  </p>
                </div>
              </div>
            </div>
                    ...
                    <div className="flex items-start gap-3.5 mb-4">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-slate-900">
                          Delivery Partner Support & Privacy Desk
                        </h3>
                        <p className="text-xs text-slate-500 mt-0.5">
                          Questions about your delivery partner profile, route tracking, or compliance records? Get in touch directly:
                        </p>
                      </div>
                    </div>
        ...
        <footer className="py-4 text-center text-xs text-[#64748B] border-t border-[#E5E7EB] bg-white px-4">
          <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
            <span>© {new Date().getFullYear()} SafeFire Delivery Operations. All rights reserved.</span>
